    ...
    match res {
        Ok(response) => {
            ...
            if !status.is_success() {
                eprintln!("Error: Server responded with status {}: {}", status, text);
                return format!("Error: Server responded with status {}", status);
            }
            text
        }
        Err(err) => {
            eprintln!("Error sending receipt: {}", err);
            "Error sending receipt".to_string()
        }
    }
}
